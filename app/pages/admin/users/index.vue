<template>
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="mb-6 flex items-center justify-between">
      ...
      <button
        class="sm:hidden w-8 h-8 flex items-center justify-center rounded-lg transition-colors"
        :class="filtersOpen ? 'bg-[#1EC3BD]/15 text-[#1EC3BD]' : 'text-gray-400 hover:bg-gray-100'"
    ...
    <div :class="filtersOpen ? 'block' : 'hidden'" class="sm:block bg-white rounded shadow-[0_2px_16px_rgba(45,77,58,0.07)] p-4 mb-6">
      <input
        v-model="search"
        type="text"
      ...
      <table class="w-full text-sm">
        <thead class="bg-[#f8faf9] border-b border-gray-100">
          <tr>
            ...
            <th class="text-left px-4 py-3 font-semibold text-[#2D4D3A] hidden lg:table-cell">Organization</th>
        ...
        <tbody class="divide-y divide-gray-50">
          <tr v-for="user in data.users" :key="user.id" class="hover:bg-[#f8faf9] transition-colors">
            <td class="px-4 py-3">
              <NuxtLink :to="`/admin/ads?author=${encodeURIComponent(user.name)}`"
                class="font-medium text-[#2D4D3A] hover:text-[#1EC3BD] hover:underline transition-colors">
                {{ user.name }}
              </NuxtLink>
            </td>
            <td class="px-4 py-3 text-[#5B5B5B] text-sm hidden lg:table-cell">{{ user.organization || '—' }}</td>
